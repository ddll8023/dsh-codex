...
/**
 * A single quota window ("5h", "week", "spark 5h", "spark week").
 * @typedef {Object} UsageWindow
 * @property {string} label - human label for the window.
...
function parseWindow(label, value, now) {
  ...
  return { label, percentage, nextResetAt };
}
...
function parseSparkWindows(root, now) {
  ...
  return [
    parseWindow("spark 5h", asRecord(rateLimit.primary_window), now),
    parseWindow("spark week", asRecord(rateLimit.secondary_window), now),
  ].filter((window) => window !== undefined);
}
...
export function formatUsageSummary(snapshot, now = Date.now()) {
  ...
  const parts = snapshot.windows.map((window) => {
    ...
    return `${window.label} ${pct}% (reset ${describeReset(window.nextResetAt, now)})`;
  });
