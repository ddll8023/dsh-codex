...
function safeMessage(error) {
  ...
  return message.length === 0 ? "login failed" : message;
}
...
function apply(ctx, config) {
  ...
  const handleCodex = async (invocation) => {
    ...
    try {
      ...
      return {
        kind: "error",
        text: `Unknown /codex subcommand "${command}".\nUsage: /codex login [--device] | /codex logout | /codex status`,
      };
    } catch (error) {
  ...
  if (commands !== undefined) {
    commands.register({
      name: "codex",
      description: "OpenAI Codex (ChatGPT Plus/Pro) account: login, logout, status",
