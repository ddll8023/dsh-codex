...
// DSH's persistence reader rejects unknown required event types before any
// session projection can run. The plugin and the host may resolve
// @deepseek-ai/dsh-session to different copies (profile vs. DSH runtime), so
// register the event in the plugin vocabulary and in the copy used by the
// host's persistence package.
async function registerSessionEventType() {
  const modules = [DshSession];
  const paths = new Set();
  const entrypoint = process.argv[1];
  if (typeof entrypoint === "string" && entrypoint.length > 0) {
    try {
      paths.add(createRequire(entrypoint).resolve("@deepseek-ai/dsh-session"));
    } catch {}
  }
  // Resolve the session package through the persistence package itself. This
  // is the copy whose vocabulary the host's reader actually imports, even
  // when the launcher entrypoint is wrapped by npx or another process.
  try {
    const persistencePath = createRequire(import.meta.url).resolve("@deepseek-ai/dsh-session-persistence");
    paths.add(createRequire(persistencePath).resolve("@deepseek-ai/dsh-session"));
  } catch {}
  for (const runtimePath of paths) {
    try {
      const runtimeSession = await import(runtimePath);
      if (runtimeSession !== DshSession) modules.push(runtimeSession);
    } catch {}
  }
  for (const sessionModule of modules) {
    const knownSessionEventTypes = sessionModule.KNOWN_SESSION_EVENT_TYPES;
    if (knownSessionEventTypes instanceof Set) knownSessionEventTypes.add(SPEED_EVENT);
  }
}
...
function apply(ctx, config) {
  ...
  // Session speed is plugin-owned. The projection is optional so the Host
  // provider remains usable in headless/minimal profiles without the projection
  // carrier; the event fold and request waterfall still work in those profiles.
  const sessionProjections = ctx.get("sessionProjections");
  if (sessionProjections !== undefined) sessionProjections.register(codexSpeedProjection);

  // Keep the Fast toggle out of DSH's provider-neutral call-config contract.
  // The request pipeline preserves this plugin-owned field through to the
  // adapter, while the durable speed event remains the source of truth.
  ctx.on("agent/request", async ({ agent }, next) => {
    const request = await next();
    if (request?.provider !== PROVIDER || agent?.session === undefined) return request;
    const speed = speedFromEvents(agent.session.events) ?? DEFAULT_SPEED;
    if (speed === "fast") return { ...request, codexServiceTier: FAST_MODE_SERVICE_TIER };
    if (request.codexServiceTier === undefined) return request;
    const { codexServiceTier: _ignored, ...standard } = request;
    return standard;
  });
  ...
  // getStatus is fast (no usage network); getUsage fetches quota separately.
  ctx.plugin(CodexAccountService, {
    getStatus: () => buildAccountStatusFast(),
    getUsage: async () => {
      // Usage is the only operation allowed to perform network I/O.
      const record = await store.read(PROVIDER).catch(() => undefined);
      if (!record) return { status: "not_logged_in" };
    ...
    try {
      ...
      if (command === "speed") {
        ...
        if (agent?.session === undefined) {
          return { kind: "error", text: "Codex speed requires an active session." };
        }
        if (rest.length > 2) {
          return { kind: "error", text: "Usage: /codex speed [standard|fast]" };
        }
        if (rest.length === 1) {
          const current = speedFromEvents(agent.session.events) ?? DEFAULT_SPEED;
          return { kind: "success", text: `Codex speed: ${speedLabel(current)}.` };
        }
        const requested = normalizeSpeed(rest[1]);
        if (requested === undefined) {
          return { kind: "error", text: "Choose Standard or Fast." };
        }
        const speed = setAgentSpeed(agent, requested);
        return { kind: "success", text: `Codex speed: ${speedLabel(speed)}.` };
      }
      ...
      return {
        kind: "error",
        text: `Unknown /codex subcommand "${command}".\nUsage: /codex login [--device] | /codex logout | /codex cancel | /codex status | /codex usage | /codex speed [standard|fast]`,
      };
    } catch (error) {
  ...
  if (commands !== undefined) {
    commands.register({
      name: "codex",
      description: "OpenAI Codex (ChatGPT Plus/Pro) account and session speed",
      input: { hint: "login [--device] | logout | status | usage | speed [standard|fast]" },
