...
function apply(ctx, config) {
  ...
  // Short-lived usage cache + inflight dedup for settings/command polling.
  let usageCache = null; // { at, value }
  let usageInflight = null;
  const USAGE_CACHE_MS = 30_000;
  async function getUsageSnapshot() {
    if (usageCache && Date.now() - usageCache.at < USAGE_CACHE_MS) return usageCache.value;
  ...
  // Serialize login start/cancel/logout so concurrent callers cannot race
  // the pendingLogin check-then-set window.
  let loginChain = Promise.resolve();
  function enqueueLogin(op) {
    const next = loginChain.then(op, op);
    loginChain = next.catch(() => {});
    return next;
  }
