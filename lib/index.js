...
function apply(ctx, config) {
  ...
  // Short-lived usage cache for settings polling (avoid hammering wham).
  ...
  ctx.plugin(CodexAccountService, {
    ...
    startLogin: async (input) => {
      ...
      // Invalidate usage cache so a new login re-fetches quota
      usageCache = null;
    ...
    cancelLogin: async () => {
      if (pendingLogin && !pendingLogin.done) {
        cancelPendingLogin("cancelled from settings");
      }
      return { ok: true };
    },
    logout: async () => {
  ...
  const handleCodex = async (invocation) => {
    ...
    try {
      ...
      if (command === "cancel") {
        if (pendingLogin === undefined || pendingLogin.done) {
          return { kind: "error", text: "No login in progress." };
        }
