window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      "speed.aria": "Codex 速度，当前 {value}",
      "speed.title": "切换 Codex 速度",
      "speed.error": "Codex 速度切换失败",
    ...
    const en = {
      "speed.aria": "Codex speed, current {value}",
      "speed.title": "Switch Codex speed",
      "speed.error": "Unable to change Codex speed",
      "speed.standard": "Standard",
      "speed.fast": "Fast",
    ...
    const css = `
      ...
      .dsh-codex-speed-root {
        display: inline-flex;
        align-items: center;
        margin-inline-start: 2px;
      }
      .dsh-codex-speed-trigger {
        min-width: 0;
        max-width: 140px;
        height: 28px;
        color: var(--dsw-alias-label-secondary);
      ...
      .dsh-codex-speed-trigger:hover:not(:disabled) {
        background: var(--dsw-alias-interactive-bg-hover);
      }
      .dsh-codex-speed-trigger:focus-visible {
        box-shadow: 0 0 0 2px var(--dsw-alias-border-l3);
      }
      .dsh-codex-speed-trigger:disabled {
        color: var(--dsw-alias-label-dimmed);
        cursor: default;
      }
      .dsh-codex-speed-label {
        min-width: 0;
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
      }
      .dsh-codex-speed-chevron {
        color: var(--dsw-alias-label-caption);
        flex: none;
        display: inline-flex;
        transition: transform 0.12s;
      }
      .dsh-codex-speed-chevron-open {
        transform: rotate(180deg);
      }
      .dsh-codex-speed-trigger[data-fast="true"] {
        color: var(--dsw-alias-label-primary);
      }
    ...
    function commandError(result, fallback = "Unable to change Codex speed") {
      if (!result?.ok) return result?.error?.message ?? fallback;
      const value = result.value;
      if (value === undefined) return fallback;
      if (value.result?.kind === "error") return value.result.text ?? fallback;
      return undefined;
    }

    function speedOptionLabel(value, t) {
      return value === "fast" ? t("speed.fast") : t("speed.standard");
    }

    function CodexSpeedControl({ sessionId, modelStore, t, useProjection, execute, locked }) {
      const modelState = React.useSyncExternalStore(modelStore.subscribe, modelStore.getSnapshot, modelStore.getSnapshot);
      const projection = useProjection("codexSpeed");
      ...
      if (!isCodex || projection === undefined || projection === null) return null;

      const current = projection.currentValue === "fast" ? "fast" : "standard";
      const options = Array.isArray(projection.options) && projection.options.length > 0
        ? projection.options
        : [{ value: "standard" }, { value: "fast" }];
      const items = options
        .filter((option) => option?.value === "standard" || option?.value === "fast")
        .map((option) => ({ id: option.value, label: speedOptionLabel(option.value, t) }));
      const currentLabel = speedOptionLabel(current, t);

      const choose = (next) => {
        setOpen(false);
        if (next === current || pending || locked) return;
        setPending(true);
        setFailed(false);
        Promise.resolve()
          .then(() => execute(sessionId, `/codex speed ${next}`, []))
          .then((result) => {
            const error = commandError(result);
            if (!alive.current) return;
            setPending(false);
            setFailed(error !== undefined);
          })
          .catch(() => {
            if (!alive.current) return;
            setPending(false);
      ...
      return React.createElement(Menu, {
        ...
        side: "top",
        className: "dsh-codex-speed-root",
        anchor: React.createElement(
          "button",
          {
            type: "button",
            className: "dsh-codex-speed-trigger",
            disabled: locked || pending,
            onClick: () => setOpen((value) => !value),
            title: failed ? t("speed.error") : t("speed.title"),
            "aria-label": t("speed.aria", { value: currentLabel }),
            "aria-haspopup": "menu",
            "aria-expanded": open,
            "aria-invalid": failed ? "true" : undefined,
            "data-fast": current === "fast" ? "true" : undefined,
          },
          React.createElement("span", { className: "dsh-codex-speed-label" }, currentLabel),
          React.createElement(
            "span",
            {
              className: open
                ? "dsh-codex-speed-chevron dsh-codex-speed-chevron-open"
                : "dsh-codex-speed-chevron",
              "aria-hidden": true,
            },
    ...
    // Re-exported from lib/protocol.js via Host; mirror light validation here for the Web bundle
    // (full schemas are declared on Host; Web validates results defensively).
    ...
    function strictAccountSchema(parse) { return { mode: "strict", typeSymbol: "dsh-codex#CodexAccountResult", schema: { parse } }; }
    function strictUsageSchema(parse) { return { mode: "strict", typeSymbol: "dsh-codex#CodexUsageResult", schema: { parse } }; }
    const CODEX_ACCOUNT_REMOTE = {
      ...
      descriptors: [
        ...
        { id: "dsh-codex#codexAccount/startLogin", service: "codexAccount", namespace: "codexAccount", method: "startLogin", invocation: { kind: "direct" }, parameters: [{ name: "input", wire: "input", source: "json", codec: { mode: "strict", typeSymbol: "dsh-codex#CodexLoginInput", schema: { parse: function(v) { if (v === null || typeof v !== "object" || Array.isArray(v)) throw new TypeError("login input must be object"); if (v.method !== "browser" && v.method !== "device_code") throw new TypeError("invalid login method"); return v; } } } }], result: strictAccountSchema(function(v) { return v; }) },
        { id: "dsh-codex#codexAccount/getUsage", service: "codexAccount", namespace: "codexAccount", method: "getUsage", invocation: { kind: "direct" }, parameters: [], result: strictUsageSchema(assertUsageResult) },
        ...
        { id: "dsh-codex#codexAccount/refreshUsage", service: "codexAccount", namespace: "codexAccount", method: "refreshUsage", invocation: { kind: "direct" }, parameters: [], result: strictUsageSchema(assertUsageResult) },
      ],
    };
    const CODEX_REMOTE = { package: "dsh-codex", descriptors: [...CODEX_USAGE_REMOTE.descriptors, ...CODEX_ACCOUNT_REMOTE.descriptors] };
    ...
    function SettingsPage({ t, accountRemote }) {
      ...
      const load = React.useCallback(async () => {
        if (!remote) { setLoading(false); setToast(t("settings.toast.requestFailed", { message: "remote not ready" })); return; }
        const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error("getStatus timeout after 5s")), 5000));
        try {
          const res = await Promise.race([remote.getStatus(), timeout]);
          if (res?.ok) {
            setStatus(res.value);
            // Fetch usage in background; don't block account render
            if (res.value.loggedIn && typeof remote.getUsage === "function") {
              setUsageLoading(true);
              const usageTimeout = new Promise((_, rej) => setTimeout(() => rej(new Error("getUsage timeout")), 8000));
              Promise.race([remote.getUsage(), usageTimeout]).then((u) => {
                if (u?.ok) setStatus((prev) => prev ? { ...prev, usage: u.value } : prev);
                else if (u && !u.ok) setStatus((prev) => prev ? { ...prev, usage: { status: "unavailable", message: u.error?.message ?? "unknown" } } : prev);
              }).catch((e) => {
                setStatus((prev) => prev ? { ...prev, usage: { status: "unavailable", message: e instanceof Error ? e.message : String(e) } } : prev);
              }).finally(() => setUsageLoading(false));
            }
          } else {
            const msg = res?.error?.message ?? "unknown";
            setToast(t("settings.toast.requestFailed", { message: msg }));
            // Still show at least the toast and stop loading so user sees something
          }
        } catch (e) {
          setToast(t("settings.toast.requestFailed", { message: e instanceof Error ? e.message : String(e) }));
          // Fallback: show empty state so page doesn't stay in "no data"
          setStatus((prev) => prev ?? { loggedIn: false, usage: { status: "not_logged_in" }, version: "0.1.0" });
        }
        finally { setLoading(false); }
      }, [remote, t]);
      React.useEffect(() => { load(); }, [load]);
      React.useEffect(() => { const id = setInterval(() => setNow(Date.now()), 30_000); return () => clearInterval(id); }, []);
      // Re-sync when tab becomes visible and cached usage is stale (>30s).
      React.useEffect(() => {
        const onVisible = () => {
          if (document.visibilityState === "visible") {
            const stale = !status?.usage?.fetchedAt || Date.now() - status.usage.fetchedAt > 30_000;
            if (stale) load();
          }
        };
        document.addEventListener("visibilitychange", onVisible);
        return () => document.removeEventListener("visibilitychange", onVisible);
      }, [load, status?.usage?.fetchedAt]);
      // During login, poll with backoff; pause when tab hidden; stop on done.
      React.useEffect(() => {
        if (!remote || !status?.pending || status.pending.done) return undefined;
        let delay = 2000;
        let timer;
        const tick = async () => {
          if (document.visibilityState !== "visible") { timer = setTimeout(tick, 5000); return; }
          try {
            const res = await remote.getStatus();
            if (res?.ok) {
              setStatus(res.value);
              if (res.value.pending?.done) return;
            }
          } catch {}
      ...
      const doRefresh = async () => {
        if (!remote) return;
        setAction("refresh"); setUsageLoading(true);
        try {
          const res = await remote.refreshUsage();
          if (!res?.ok) throw new Error(res?.error?.message ?? "failed");
          // refreshUsage returns usage; also refresh account in background
          if (res.value) setStatus((prev) => prev ? { ...prev, usage: res.value } : prev);
          const st = await remote.getStatus();
          if (st?.ok) setStatus((prev) => ({ ...st.value, usage: res.value ?? st.value.usage }));
        } catch (e) { setToast(t("settings.toast.usageFailed", { message: e instanceof Error ? e.message : String(e) })); }
        finally { setAction(""); setUsageLoading(false); }
      };
      ...
      return React.createElement("div", { className: "dsh-codex-settings" },
        ...
        React.createElement("div", { className: "dsh-codex-card" },
          ...
          usage?.status === "loading" && React.createElement("p", { className: "dsh-codex-meta" }, usageLoading ? t("settings.usage.loading") : t("settings.usage.loading")),
    ...
    const inject = ["slots", "remote", "remote.commands", "locale", "modelDirectories"];
    ...
    async function apply(ctx) {
      await ctx.inject(inject, async (scope) => {
        ...
        ensureSettingsCss();
        const disposeRemote = await scope.remote.$mount(CODEX_REMOTE);
        const usageRemote = scope.get("remote.codexUsage");
        if (usageRemote === undefined) { await disposeRemote(); throw new Error("dsh-codex: usage Remote did not mount"); }
        const accountRemote = scope.get("remote.codexAccount");
        if (accountRemote === undefined) { await disposeRemote(); throw new Error("dsh-codex: account Remote did not mount"); }
        const usageStore = new UsageStore(usageRemote);
        scope.effect(() => async () => { usageStore.dispose(); await disposeRemote(); }, "dsh-codex: UI cleanup");
          ...
          scope.slots.register(
            {
              ...
              inject: (sessionId) => {
                ...
                return { modelStore: directory.store };
              },
            },
            (props) => React.createElement(CodexSpeedControl, {
              ...props,
              t,
              execute: (sessionId, line, args) => scope.remote.commands.execute(sessionId, line, args),
            }),
          ),
        );
        ...
        scope.slots.inject("settings.section", () =>
          scope.slots.register(
            ...
            () => React.createElement(SettingsPage, { t, accountRemote }),
          ),
        );
      });
    }
