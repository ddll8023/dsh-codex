...
import { CODEX_WEB_SEARCH_MODES } from "./web-search.js";
...
export const Config = z.object({
  ...
  webSearchMode: z.union(["live", "cached", "indexed", "disabled"]).default("live"),
...
export function resolveAdapterOptions(config) {
  ...
  const nativeWebSearch = config.nativeWebSearch ?? true;
  if (typeof nativeWebSearch !== "boolean") throw new Error("dsh-codex: nativeWebSearch must be a boolean");
  const webSearchMode = config.webSearchMode ?? "live";
  if (!CODEX_WEB_SEARCH_MODES.includes(webSearchMode)) {
    throw new Error(`dsh-codex: webSearchMode must be one of ${CODEX_WEB_SEARCH_MODES.join(", ")}`);
  }
