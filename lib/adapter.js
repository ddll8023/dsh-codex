...
import { FAST_MODE_SERVICE_TIER } from "./speed.js";
...
/** Resolve the plugin-owned request field before it reaches pi-ai. */
function resolveServiceTier(options) {
  const tier = options.codexServiceTier;
  if (tier === undefined) return undefined;
  if (tier !== FAST_MODE_SERVICE_TIER) {
    throw new LlmError(`dsh-codex does not support service tier "${String(tier)}"`, "UNSUPPORTED_OPTION");
  }
  return tier;
}
...
export class CodexAdapter extends LlmAdapter {
  ...
  listModels(provider) {
    return Promise.resolve().then(() => {
      ...
      return models.getModels(provider).map((model) => {
        const override = conn.models.find((entry) => entry.id === model.id);
        return {
          provider,
          id: model.id,
  ...
  resolveModel(provider, model, _signal) {
    return Promise.resolve().then(() => {
      ...
      const override = conn.models.find((entry) => entry.id === model);
      const defaultLevel = describableReasoningLevel(resolved, conn.reasoning);
      return {
        provider,
        id: model,
