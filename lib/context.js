...
/** Convert the harness tool declarations into pi-ai context tools. */
...
function splitResults(content) {
  const results = content.filter((b) => b.type === "tool-result");
  return results;
}
...
function textOnlyContext(options) {
  ...
  for (const message of options.messages) {
    ...
    const results = splitResults(message.content);
