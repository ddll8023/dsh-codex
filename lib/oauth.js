...
/** Maximum wall-clock time a browser login parks on manual_code (10 min). */
export const MANUAL_CODE_TIMEOUT_MS = 10 * 60 * 1000;
  ...
  return {
    ...
    async prompt(prompt) {
      ...
      if (prompt.type === "manual_code") {
        ...
        // cancels it. Upstream pi-ai races server.waitForCode() vs this
        // prompt; we add a wall-clock timeout so the parked promise never
        // leaks indefinitely when neither callback nor abort ever fires.
        return new Promise((_resolve, reject) => {
          ...
          }, MANUAL_CODE_TIMEOUT_MS);
          // Node's setTimeout keeps the loop alive; don't block process exit.
          timer.unref?.();
        });
      }
 ...
 * @param getUsage - optional cached usage fetcher (for Host-side dedup); when
 *   omitted the legacy direct fetch path is used (kept for unit tests).
 * @returns a usage line, or `undefined` when not logged in.
 */
export async function describeUsage(models, store, getUsage) {
  if (getUsage) {
    const cached = await getUsage();
    if (cached.status === "not_logged_in") return undefined;
    if (cached.status === "unavailable") return `Usage: unavailable (${cached.message ?? "unknown"})`;
    return formatUsageSummary(cached);
  }
 ...
 * @param getUsage - optional cached usage fetcher (Host dedup).
 * @returns the report text.
 */
export async function describeStatus(models, store, pending, getUsage) {
  const lines = [];
  ...
  const usage = await describeUsage(models, store, getUsage);
